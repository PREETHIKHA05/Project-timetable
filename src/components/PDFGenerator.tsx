...
export const PDFGenerator: React.FC = () => {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      pdf.text("CHENNAI INSTITUTE OF TECHNOLOGY", pageWidth / 2, yPosition, {
        align: "center",
      });
      ...
      pdf.setFontSize(12);
      pdf.setFont("helvetica", "bold");
      pdf.text("EXAMINATION SCHEDULE", pageWidth / 2, yPosition, {
        align: "center",
      });

      yPosition += 8;

      // Create examination schedule data (only scheduled exam dates)
      const scheduleData = createExamSchedule().filter(
      ...
      departments.forEach(
        (dept: { code: string; name: string }, index: number) => {
      ...
      scheduleData.forEach((row: any, rowIndex: number) => {
        ...
        // Department columns
        departments.forEach(
          (dept: { code: string; name: string }, deptIndex: number) => {
      ...
      pdf.setFont("helvetica", "bold");
      pdf.setFontSize(12);
      pdf.text("Dr. A. RAMESH, M.E., Ph.D.", pageWidth - 80, sigY, {
        align: "center",
      });
      pdf.setFontSize(10);
      ...
      // Save the PDF
      const filename = `CIT_${examTypeText.replace(
        /\s+/g,
        "_"
      )}_${yearText.replace(/\s+/g, "_")}_${
  ...
  const getSemesterForYear = (year: string) => {
    switch (year) {
      case "2":
        return 4; // II Year - 4th semester
      case "3":
        return 6; // III Year - 6th semester
      case "4":
        return 8; // IV Year - 8th semester
      default:
        return 8;
    }
  };
  ...
  const createExamSchedule = () => {
    ...
    const filteredExams = scheduledExams.filter((exam) => {
      ...
      return (
        exam.semester === undefined ||
        yearToSemester[selectedYear]?.includes(exam.semester) ||
        exam.year === parseInt(selectedYear)
      );
    });

    // Only use actual scheduled exam dates
    const uniqueDates = Array.from(
      new Set(filteredExams.map((exam) => exam.examDate))
    ).sort();
    uniqueDates.forEach((dateStr) => {
      ...
      const subjects: {
        [key: string]: { code: string; name: string; time?: string };
      } = {};
      const examsOnDate = filteredExams.filter(
        (exam) => exam.examDate === dateStr
      );
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full text-xs border border-green-200 rounded-lg">
                      ...
                      <tbody>
                        {createExamSchedule().map((row, idx) => (
