  ...
  async scheduleExam(subjectId: string, examDate: string, assignedBy: string, selectedExamType: 'IA1' | 'IA2' | 'IA3'): Promise<void> {
    ...
    if (subjectId.startsWith('staff-subject-')) {
      ...
      if (usersError) {
        ...
        console.log('auth.users IDs:', users.map(u => u.id));
      }
      ...
      for (const schedule of schedulesToCreate) {
        ...
        if (existingSchedule) {
          ...
          throw new Error(`This subject is already scheduled for this department on ${existingSchedule.exam_date}. Each subject can only be scheduled once per department.`);
        } else {
          // Create new schedule
          ...
          if (insertError) {
            ...
            if (insertError.message.includes('duplicate key value violates unique constraint') && 
                insertError.message.includes('exam_schedules_subject_id_department_id_key')) {
              throw new Error(`This subject is already scheduled for this department. Each subject can only be scheduled once per department.`);
            }
        ...
        otherDeptSubjects.forEach(subject => {
          ...
          if (deptId) {
            schedulesToCreate.push({
              ...
              department_id: deptId,
              assigned_by: assignedBy,
              priority_department: currentDepartmentId, // Mark original department as priority
              exam_type: selectedExamType
            });
          }
        });
      ...
      // Notify other departments about the shared subject scheduling
      if (otherDeptSubjects && otherDeptSubjects.length > 0) {
        await this.notifySharedSubjectScheduling(subject.name, examDate, subject.department);
      }
